...
public class CtrlTablero {
    ...
    public static Tablero loadFromFile(String filename)
    {
        ...
        // Leer el tama√±o del tablero (primera l√≠nea):
        ...
        for (int i = 0; i < filas; ++i)
        {
            for (int j = 0; j < columnas; ++j)
            {
                ...
                {
                    if (texto.charAt(0) >= '0' && texto.charAt(0) <= '9') // Si es un d√≠gito, quiero leer el n√∫mero completo:
                    {
                        pointer = 0;
                        while (pointer < texto.length() && texto.charAt(pointer) >= '0' && texto.charAt(pointer) <= '9')
                            ++pointer;
                        c = new CasillaBlanca(Integer.parseInt(texto.substring(0,pointer))); // Casilla blanca con el valor del n√∫mero
                    }
    ...
    public static Boolean saveToFile(Tablero tablero, String filename, Boolean sobreescribir_archivo)
    {
        ...
        int[] tamaÒo = tablero.getTamaÒo();

        // Imprimir tama√±o:
        file.writeNextLine(tamaÒo[0] + "," + tamaÒo[1]);

        // Imprimir l√≠nea por l√≠nea:
        for (int i = 0; i < tamaÒo[0]; ++i)
        {
        ...
        // Aqui faltaria a√±adir imprimir el tiempo:
        //  ...
