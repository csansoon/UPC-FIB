...
{
    models = {"Patricio", "HomerProves", "Homer", "bunny", "dolphin", "f-16", "legoman", "legoman-assegut"};
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_L: {
        setIluminacio(not iluminacio);
        break;
    }
    case Qt::Key_M: {
        createBuffersPatricio("./models/Patricio.obj");
        break;
    }
...
void MyGLWidget::createBuffersPatricio (std::string mod)
{
...
void MyGLWidget::carregaShaders()
{
  ...
  ilumLoc = glGetUniformLocation (program->programId(), "ilum");
  scoLoc = glGetUniformLocation (program->programId(), "SCO");
  posFocusLoc = glGetUniformLocation (program->programId(), "posFocus");
  colFocusLoc = glGetUniformLocation (program->programId(), "colFocus");
}

void MyGLWidget::setSCO(bool newSCO) {
    SCO = newSCO;
    int uintSCO = 0;
    if (SCO) uintSCO = 1;
    glUniform1i(scoLoc, uintSCO);
    
    emit actualitza_SCO(SCO);
}

void MyGLWidget::setIluminacio(bool iluminacioEncesa) {
    iluminacio = iluminacioEncesa;
    
    int ilum = 0;
    if (iluminacio) ilum = 1;

    glUniform1i(ilumLoc, ilum);
    
    emit actualitza_llum(iluminacio);
    setSCO(iluminacioEncesa);
}

void MyGLWidget::setModel(int index) {
    while (index < 0) index = models.size() + index;
    while (index >= models.size()) index = index - models.size();
    
    modelIndex = index;
    createBuffersPatricio("./models/" + models[modelIndex] + ".obj");
    //QString nom_model = models[modelIndex];
    QString nom_model = QString::fromUtf8(models[modelIndex].c_str());
    emit actualitza_nom_model(nom_model);
}

void MyGLWidget::activa_llum(const bool & checkBox) {
    makeCurrent();
    setIluminacio(checkBox);
    update();
}

void MyGLWidget::next_model() {
    makeCurrent();
    setModel(++modelIndex);
    update();
}

void MyGLWidget::previous_model() {
    makeCurrent();
    setModel(--modelIndex);
    update();    
}
