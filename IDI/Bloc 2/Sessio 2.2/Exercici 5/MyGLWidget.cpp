...
void MyGLWidget::escenaInicial() {
    ...
    //OBS = glm::vec3(0, -1, radi_escena * 2);
    VRP = centre_escena;
    UP = glm::vec3(0, 1, 0);
    
    
    float distancia = distance(centre_escena, OBS);
...
void MyGLWidget::projectTransform () 
{
    ...
    Proj = glm::ortho (-radi_escena, radi_escena, -radi_escena, radi_escena, znear, zfar);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_O: {
        if (axonometric) axonometric = false;
        else axonometric = true;
        ini_camera();
        break;
    }
...
void MyGLWidget::calculaCapsaEscena()
{
  ...
  //if (minimModel.x < minimEscena.x) minimEscena.x = minimModel.x;
  //if (minimModel.y < minimEscena.y) minimEscena.y = minimModel.y;
  //if (minimModel.z < minimEscena.z) minimEscena.z = minimModel.z;
  //if (maximModel.x > maximEscena.x) maximEscena.x = maximModel.x;
  //if (maximModel.y > maximEscena.y) maximEscena.y = maximModel.y;
  //if (maximModel.z > maximEscena.z) maximEscena.z = maximModel.z;
  maximEscena.y = 4.0;
}
